...
/**
 * Signature manuscrite « hello » / « xin chào » qui s’écrit trait par trait.
 *
 * Même API, mêmes tracés et mêmes cadences que la version d’origine à base de
 * `motion`, mais l’animation est écrite à la main : chaque trait est révélé
 * par `stroke-dashoffset` (voir `.hello-stroke` dans globals.css), avec
 * `pathLength="1"` pour raisonner en proportion du tracé plutôt qu’en pixels.
 *
 * Pourquoi : `motion` pesait 45 kB gzip et faisait passer la page d’accueil
 * de 177 à 232 kB, au-dessus du budget de 200 kB du projet. Le rendu est
 * identique, le coût JavaScript est nul, et `prefers-reduced-motion` se
 * traite par une simple requête média.
 */

type Stroke = {
  d: string;
  /** Secondes, à la vitesse 1 — multipliées par `speed`. */
  duration: number;
  delay: number;
...
const VIETNAMESE_STROKES: Stroke[] = [
  // x1
  {
...
type Props = Omit<React.ComponentProps<"svg">, "onAnimationEnd"> & {
  speed?: number;
  /** Appelé quand le dernier trait est terminé. */
...
function HelloStrokes({
  ...
  onAnimationComplete,
  ...props
}: Props & { strokes: Stroke[]; viewBox: string; title: string }) {
  return (
    <svg
    ...
    >
      ...
      {strokes.map((stroke, i) => (
        <path
          key={i}
          className={cn("hello-stroke", stroke.className)}
          d={stroke.d}
          pathLength={1}
          style={{
          ...
          // Le dernier trait porte la fin de l’animation d’ensemble. Le
          // marqueur permet à un parent d’écouter l’événement qui remonte,
          // sans avoir à recevoir de fonction — ce qui autorise le rendu
          // serveur de ce SVG sous un composant client (voir HelloIntro).
          data-hello-last={i === strokes.length - 1 ? "" : undefined}
          onAnimationEnd={i === strokes.length - 1 ? onAnimationComplete : undefined}
        />
      ))}
    </svg>
  );
...
function AppleHelloVietnameseEffect(props: Props) {
  return (
    <HelloStrokes
      {...props}
      strokes={VIETNAMESE_STROKES}
...
function AppleHelloEnglishEffect(props: Props) {
  return (
    <HelloStrokes
      {...props}
      strokes={ENGLISH_STROKES}
      viewBox="0 0 638 200"
      title="hello"
    />
  );
